...
#define StructSize sizeof(CircularVector)
#define VectorPaddingScale 1
...
#define GetFromRawPos(vector, position) \
	vector->storage_[(vector->head_ + position) % vector->capacity_]
...
int main() {
	...
	//Create:
	v = CIRCULARVECTOR_create(0);
	assert(NULL == v);
	PrintTest("Vector_create #1              ");

	v = CIRCULARVECTOR_create(kVectorCapacity_01);
	assert(NULL != v);
	assert(NULL != v->storage_);
	assert(NULL != v->ops_);
	assert(v->head_ == v->tail_);
	assert(kVectorCapacity_01 * VectorPaddingScale == v->capacity_);
	for (u16 i = 0; i < kVectorCapacity_01 * VectorPaddingScale; ++i) {
	...
	CircularVector* v_02 = CIRCULARVECTOR_create(kVectorCapacity_02);
	assert(NULL != v_02);
	assert(NULL != v_02->storage_);
	assert(NULL != v_02->ops_);
	assert(v_02->head_ == v_02->tail_);
	assert(kVectorCapacity_02 * VectorPaddingScale == v_02->capacity_);
	for (u16 i = 0; i < kVectorCapacity_02 * VectorPaddingScale; ++i) {
	...
	CircularVector* v_03 = CIRCULARVECTOR_create(kVectorCapacity_03);
	assert(NULL != v_03);
	assert(NULL != v_03->storage_);
	assert(NULL != v_03->ops_);
	assert(v_03->head_ == v_03->tail_);
	assert(kVectorCapacity_03 * VectorPaddingScale == v_03->capacity_);
	for (u16 i = 0; i < kVectorCapacity_03 * VectorPaddingScale; ++i) {
	...
	//********************* STATUS CHECK:
	expected_structs += 3;
	expected_memnodes += kVectorCapacity_01 * VectorPaddingScale;
	expected_memnodes += kVectorCapacity_02 * VectorPaddingScale;
	expected_memnodes += kVectorCapacity_03 * VectorPaddingScale;
	CheckMMUsageWithSizeof(expected_data, expected_structs, expected_memnodes)
	MM->status();
	...
	cmp_output = strncmp(GetFromRawPos(v, 0).data_, q_00, 4);
	assert(0 == cmp_output);
	PrintTest("Vector_insertFirst #Check 0   ");

	cmp_output = strncmp(GetFromRawPos(v, 1).data_, q_01, 4);
	assert(0 == cmp_output);
	PrintTest("Vector_insertFirst #Check 1   ");

	cmp_output = strncmp(GetFromRawPos(v, 2).data_, q_02, 4);
	assert(0 == cmp_output);
	PrintTest("Vector_insertFirst #Check 2   ");

	cmp_output = strncmp(GetFromRawPos(v, 3).data_, q_03, 3);
	assert(0 == cmp_output);
	PrintTest("Vector_insertFirst #Check 3   ");

	cmp_output = strncmp(GetFromRawPos(v, 4).data_, q_04, 3);
	assert(0 == cmp_output);
	...
	cmp_output = strncmp(GetFromRawPos(v, 5).data_, q_05, 5);
	assert(0 == cmp_output);
	PrintTest("Vector_insertLast #Check 0    ");

	cmp_output = strncmp(GetFromRawPos(v, 6).data_, q_06, 5);
	assert(0 == cmp_output);
	PrintTest("Vector_insertLast #Check 1    ");

	cmp_output = strncmp(GetFromRawPos(v, 7).data_, q_07, 5);
	assert(0 == cmp_output);
	PrintTest("Vector_insertLast #Check 2    ");

	cmp_output = strncmp(GetFromRawPos(v, 8).data_, q_08, 5);
	assert(0 == cmp_output);
	PrintTest("Vector_insertLast #Check 3    ");

	cmp_output = strncmp(GetFromRawPos(v, 9).data_, q_09, 6);
	assert(0 == cmp_output);
	...
	cmp_output = strncmp(GetFromRawPos(v, 5).data_, q_14, 6);
	assert(0 == cmp_output);
	PrintTest("Vector_insertAt #Check 0      ");

	cmp_output = strncmp(GetFromRawPos(v, 6).data_, q_13, 5);
	assert(0 == cmp_output);
	PrintTest("Vector_insertAt #Check 1      ");

	cmp_output = strncmp(GetFromRawPos(v, 7).data_, q_12, 4);
	assert(0 == cmp_output);
	PrintTest("Vector_insertAt #Check 2      ");

	cmp_output = strncmp(GetFromRawPos(v, 8).data_, q_11, 3);
	assert(0 == cmp_output);
	PrintTest("Vector_insertAt #Check 3      ");

	cmp_output = strncmp(GetFromRawPos(v, 9).data_, q_10, 2);
	assert(0 == cmp_output);
	PrintTest("Vector_insertAt #Check 4      ");

	cmp_output = strncmp(GetFromRawPos(v, 15).data_, q_15, 7);
	assert(0 == cmp_output);
	...
	//************************************
	//********************* STATUS CHECK:
	// No memory should have changed at this point
	CheckMMUsageWithSizeof(expected_data, expected_structs, expected_memnodes)
	MM->status();
	...
	assert(kErrorCode_Ok == err_code);
	assert(9 * VectorPaddingScale == v_02->capacity_);
	cmp_output = strncmp(GetFromRawPos(v_02, 0).data_, mn_01, 6);
	assert(0 == cmp_output);
	cmp_output = strncmp(GetFromRawPos(v_02, 1).data_, mn_02, 5);
	assert(0 == cmp_output);
	cmp_output = strncmp(GetFromRawPos(v_02, 2).data_, mn_03, 11);
	assert(0 == cmp_output);
	cmp_output = strncmp(GetFromRawPos(v_02, 3).data_, mn_04, 23);
	assert(0 == cmp_output);
	assert(GetFromRawPos(v_02, 3).data_ != GetFromRawPos(v_03, 0).data_);
	cmp_output = strncmp(GetFromRawPos(v_02, 4).data_, mn_05, 28);
	assert(0 == cmp_output);
	assert(GetFromRawPos(v_02, 4).data_ != GetFromRawPos(v_03, 1).data_);
	...
	err_code = v->ops_->resize(v, v->ops_->capacity(v));
	assert(kErrorCode_Ok == err_code);
	...
	for (u16 i = 0; i < kVectorCapacity_01; ++i) {
		addr_data[i] = GetFromRawPos(v, i).data_;
	}
	...
	assert(NULL != v->ops_);
	assert(kVectorCapacity_01_01 * VectorPaddingScale == v->capacity_);
	for (u16 i = 0; i < kVectorCapacity_01; ++i) {
		assert(GetFromRawPos(v, i).data_ == addr_data[i]);
		assert(GetFromRawPos(v, i).ops_);
	}
	for (u16 i = kVectorCapacity_01; i < kVectorCapacity_01_01; ++i) {
		assert(GetFromRawPos(v, i).data_ == NULL);
		assert(GetFromRawPos(v, i).ops_);
	}
	...
	// Changed v capacity
	expected_memnodes += kVectorCapacity_01_01 * VectorPaddingScale;
	expected_memnodes -= kVectorCapacity_01 * VectorPaddingScale;
	CheckMMUsageWithSizeof(expected_data, expected_structs, expected_memnodes)
	...
	assert(storage_addr != v->storage_);
	assert(NULL != v->ops_);
	assert(kVectorCapacity_01_02 * VectorPaddingScale == v->capacity_);
	...
	// We should lose memnodes and the data of the filled ones.
	expected_data -= resize_data_loss;
	expected_memnodes += kVectorCapacity_01_02 * VectorPaddingScale;
	expected_memnodes -= kVectorCapacity_01_01 * VectorPaddingScale;
	CheckMMUsageWithSizeof(expected_data, expected_structs, expected_memnodes)
	...
	// We should lose all the data from v.
	...
	err_code = v->ops_->insertLast(v, data.full_test_pack.ptr, 
											           data.full_test_pack.size);
	assert(kErrorCode_Ok == err_code);
	assert((v->head_ + 1) % v->capacity_ == v->tail_);
	assert(NULL != GetFromRawPos(v, 0).data_);
	assert(data.full_test_pack.ptr == GetFromRawPos(v, 0).data_);
	assert(kVectorCapacity_01_02 * VectorPaddingScale == v->capacity_);
